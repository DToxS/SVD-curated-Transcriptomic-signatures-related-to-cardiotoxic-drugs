...
topDEGs_for_MBCO_application=600
...
{#Begin
  ...
  tasks = rbind(regular_tasks,tasks)
}#End
else { tasks = regular_tasks }}
...
{#Begin
  ...
  clusterCall(parallel_clusters[indexCore], function(d) {assign('topDEGs_for_MBCO_application', d, pos=.GlobalEnv)}, topDEGs_for_MBCO_application)
}#End
...
Generate_input_gene_lists_for_MBCO_PathNet = function(gene_lines,addTo_integrationGroup,topDEGs_for_MBCO_application)
{#Begin - Generate_input_gene_lists_for_MBCO_PathNet
   source('SVD_colors.R')
   missingColor_color = "gray60"
   mbco_input = gene_lines
   indexKeep = which(mbco_input$Value!=0)
   mbco_input = mbco_input[indexKeep,]
   mbco_input[["Dataset name"]] = paste(mbco_input$Drug,"-",mbco_input$Drug_class,"-",gsub("Cell_line.","",mbco_input$Cell_line),"-",addTo_integrationGroup,sep='')
   mbco_input[["Timepoint"]] = 0
   mbco_input[["Timeunit"]] = "min"
   mbco_input[["Integration group"]] = paste(addTo_integrationGroup,"-",mbco_input$Drug_class,sep='')
   indexUp = which(mbco_input$Value>0)
   indexDown = which(mbco_input$Value<0)
   mbco_input[["Integration group"]][indexUp] = paste(mbco_input[["Integration group"]][indexUp],"-Up",sep='')
   mbco_input[["Integration group"]][indexDown] = paste(mbco_input[["Integration group"]][indexDown],"-Down",sep='')
   mbco_input[["Dataset color"]] = missingColor_color
   mbco_input[["NCBI official gene symbol"]] = toupper(mbco_input$Gene_symbol)
   mbco_input$Gene_symbol = NULL
   mbco_input[["Value_1st"]] = mbco_input$Value
   mbco_input$Value = NULL
   mbco_input[["Value_2nd"]] = 0
   drugs = unique(mbco_input$Drug)
   drug_colors = replicate(length(drugs),missingColor_color)
   names(drug_colors) = drugs
   indexDrug=1
   for (indexDrug in 1:length(drugs))
   {#Begin
      drug = drugs[indexDrug]
      indexCurrentColor = which(drugs_for_highlight_colors==drug)
      if (length(indexCurrentColor)==1)
      {#Begin
         drug_colors[indexDrug] = col2hex(drug_highlight_colors[indexCurrentColor])
      }#End
   }#End
   drug_colors = Fill_missing_colors(drug_colors,missingColor_color)
   for (indexDrug in 1:length(drugs))
   {#Begin
     drug = drugs[indexDrug]
     indexCurrentColor = which(names(drug_colors)==drug)
     indexCurrentMbco = which(mbco_input$Drug==drug)
     mbco_input[["Dataset color"]][indexCurrentMbco] = drug_colors[indexCurrentColor]
   }#End
   mbco_input$Drug_class = NULL
   mbco_input$Drug = NULL
   mbco_input$Cell_line = NULL
   #stopifnot(length(unique(mbco_input$Plate))==1)
   mbco_input$Plate = NULL
   stopifnot(length(unique(mbco_input$Reference_valueType))==1)
   mbco_input$Reference_valueType = NULL
   stopifnot(length(unique(mbco_input$EntityClass))==1)
   mbco_input$EntityClass = NULL
   stopifnot(length(unique(mbco_input$Correlation_method))==1)
   mbco_input$Correlation_method = NULL
   stopifnot(length(unique(mbco_input$Preprocess_data))==1)
   mbco_input$Preprocess_data = NULL
   stopifnot(length(unique(mbco_input$Decomposition_method))==1)
   mbco_input$Decomposition_method = NULL
   stopifnot(length(unique(mbco_input$EA_correlation_parameter))==1)
   mbco_input$EA_correlation_parameter = NULL
   stopifnot(length(unique(mbco_input$Minimum_fraction_of_max_f1score))==1)
   mbco_input$Minimum_fraction_of_max_f1score = NULL
   stopifnot(length(unique(mbco_input$Dataset))==1)
   mbco_input$Dataset = NULL
   dataset_names = unique(mbco_input[["Dataset name"]])
   mbco_input$Significant = FALSE
   mbco_input$Rank = -1
   for (indexDN in 1:length(dataset_names))
   {#Begin
       dataset_name = dataset_names[indexDN]
       indexCurrentDN = which(mbco_input[["Dataset name"]] == dataset_name)
       mbco_input$Rank[indexCurrentDN] = rank(-abs(mbco_input$Value_1st[indexCurrentDN]))
   }#End
   indexKeep = which(mbco_input$Rank<=topDEGs_for_MBCO_application)
   mbco_input = mbco_input[indexKeep,]
   return (mbco_input)
}#End - Generate_input_gene_lists_for_MBCO_PathNet

Get_readMeText_for_use_of_MBCO_PathNet = function()
{#Begin - Get_readMeText_for_use_of_MBCO_PathNet
  readMe_text = c("For quick pathway enrichment analysis and network-based integration of complete and",
                  "drug-selective gene expression profiles, download our MBCO PathNet desktop application",
                  "at www.mbc-ontology.org (https://github.com/SBCNY/Molecular-Biology-of-the-Cell).",
                  "",
                  "Open the application and open the 'Read data' menu. Select 'MBCO' as 'Default column names'",
                  "and 'all files in directory' as the 'Read' instructions. Copy paste the complete name of the",
                  "directory './14_DEGs_MBCO_PathNet/' into the 'Read all data files in directory' text box.",
                  "Press the 'Read'- button. Pressing the 'Analyze'-button will subject the data to enrichment",
                  "analysis and generate the pathway networks that can be found in the specified results folder.",
                  "Since generation of the figures and writing of the network files are the most time-consumptive",
                  "steps, we instructed the application within the generated parameter settings file to only",
                  "write the network files. Use the menus 'Enrichment' and 'SCP networks' to change those",
                  "instructions. Results will be saved in the specified results folder and can be investigated",
                  "while the application is till writing the other results files.",
                  "",
                  "Our appliation writes SCP networks. Each SCP is visualized as pie charts and each slice",
                  "represents a datasets for which that SCP was predicted with significance. We set in the",
                  "parameter settings file that SCP nodes and slices should be proportional to the significance",
                  "values obtained for each SCP within each dataset (i.e., -log10(p-values). In case of large",
                  "variations in the SCP significances across the different datasets, this might cause that some",
                  "SCP nodes are very small. In such a case, you can change the 'SCP node areas' determinant in",
                  "the menu 'SCP networks'.",
                  "",
                  "All datasets assigned to the same integration group will be part of the same network. To",
                  "quickly change the integration group assignments, open the 'Organize data' menu. Select",
                  "'Substring(s) in name' and enter '-' as the substring delimiter. Defined 'Substring no(s)'",
                  "from the left and/or right side of the name will allow temporal grouping of all datasets",
                  "with identical substrings at the defined positions, so they can be assigned to the same",
                  "integration group. Integration group assignment can be down automatically by the application,",
                  "using the related button. The dataset names in this folder are composed of the following substrings",
                  "(from left to right): 1. drug, 2. drug class, 3. cell line, 4. status of SVD processing.",
                  "There are three possible statuses of SVD processing: complete = complete DEGs, no1stEA: DEGs after",
                  "removal of any selected eigenarrays ('no1stEA' is just the initially selected name that is",
                  "independent of the removed eigenarray(s)), dS: drug-selective DEGs. For example, if you want to",
                  "compare all drug-selective results obtained for the same cell line, enter '3,4' in the 'left'",
                  "text box. It is generally recommended to select 'Up/Down' as well, which will additionally",
                  "consider the direction of change for assignment of datasets into integration groups. Dataset",
                  "colors can be quickly assigned as well using the same functionality.")
  return (readMe_text)
}#End - Get_readMeText_for_use_of_MBCO_PathNet

Get_MBCOAppParameterSettings_for_use_of_MBCO_PathNet = function()
{#Begin - Get_MBCOAppParameterSettings_for_use_of_MBCO_PathNet
  parameterSettings_text = c("#MBCO parameter settings for re-import into application",
                             "User_data_options_class\tSignificance_definition_value_1st\tHigher_abs_values_are_more_significant",
                             "User_data_options_class\tAll_genes_significant\tTrue",
                             "MBCO_enrichment_pipeline_options_class\tNext_ontology\tMbco_human",
                             "MBCO_network_based_integration_options_class\tMax_diameter\tMinus_log10_pvalue\t200",
                             "MBCO_network_based_integration_options_class\tLabel_reference_size\tMinus_log10_pvalue\t15",
                             "MBCO_network_based_integration_options_class\tNode_size_scaling_across_plots\tUnique",
                             "MBCO_network_based_integration_options_class\tAdjust_labelSizes_to_nodeSizes\tTrue",
                             "MBCO_network_based_integration_options_class\tGenerate_scp_networks\tTrue",
                             "MBCO_network_based_integration_options_class\tNode_size_determinant\tMinus_log10_pvalue",
                             "Bardiagram_options_class\tGenerate_bardiagrams\tFalse",
                             "Bardiagram_options_class\tCustomized_colors\tTrue",
                             "Heatmap_options_class\tGenerate_heatmap\tFalse",
                             "Timeline_options_class\tGenerate_timeline\tFalse")
  return (parameterSettings_text)
}#End - Get_MBCOAppParameterSettings_for_use_of_MBCO_PathNet
...
results_for_mbco_app_were_written = FALSE
        ...
        {#Begin
           ...
           {#Begin
             ...
             {#Begin
                ...
                {#Begin
                   Data_for_r_gene_expression = Data_current[,indexCurrentEntityColumns,drop=FALSE]
                   rm(Data_current)
                   delimiter_that_separates_colnames = "[.]"
                   col_splitString_indexDrug = 3
                   col_splitString_indexCellline = 2 
                   ...
                   {#Begin
                      ...
                      {#Begin
                         ...
                         indexOutlierInROutput = which(gsub("Cell_line.","",r_output_gene_expression_lines$Cell_line)==outlier_cellline)
...
if (length(original_gene_lines[,1])>0)
{#Begin
  ...
  tryCatch(
    {#Begin - Try
       bgGenes = toupper(unique(original_gene_lines$Gene_symbol))
       addTo_integrationGroup = mbco_original_term
       mbco_app_input_lines = Generate_input_gene_lists_for_MBCO_PathNet(gene_lines = original_gene_lines, addTo_integrationGroup = addTo_integrationGroup, topDEGs_for_MBCO_application = topDEGs_for_MBCO_application)
       write.table(mbco_app_input_lines,file=complete_completeDEGs_for_MBCO_PathNet_fileName,row.names=FALSE,col.names=TRUE,quote=FALSE,sep='\t')
       write.table(bgGenes,file=complete_completeDEGs_for_MBCO_PathNet_bgGenes_fileName,row.names=FALSE,col.names=TRUE,quote=FALSE,sep='\t')
       results_for_mbco_app_were_written = TRUE
       rm(mbco_app_input_lines)
    },#End - Try
    error = function(e)
    {#Begin - error
      message('File for MBCO PathNet not written, most likely the data did not fulfill the requirements within the Generate_input_gene_lists_for_MBCO_PathNet function.')
      print(e)
    }#End - error
  )
...
if (length(drug_specific_gene_lines[,1])>0)
{#Begin
   ...
   {#Begin
      ...
      {#Begin
          ...
          tryCatch(
            {#Begin - Try
              bgGenes = toupper(unique(drug_specific_gene_lines$Gene_symbol))
              mbco_app_input_lines = Generate_input_gene_lists_for_MBCO_PathNet(gene_lines = fate_drug_specific_gene_lines, addTo_integrationGroup = "dS", topDEGs_for_MBCO_application = topDEGs_for_MBCO_application)
              write.table(mbco_app_input_lines,file=complete_drugSelectiveDEGs_for_MBCO_PathNet_fileName,row.names=FALSE,col.names=TRUE,quote=FALSE,sep='\t')
              write.table(bgGenes,file=complete_drugSelectiveDEGs_for_MBCO_PathNet_bgGenes_fileName,row.names=FALSE,col.names=TRUE,quote=FALSE,sep='\t')
              results_for_mbco_app_were_written = TRUE
              rm(mbco_app_input_lines)
            },#End - Try
            error = function(e)
...
if (results_for_mbco_app_were_written)
{#Begin - Write MBCO PathNet ReadMe file and parameter settings file
   text_lines = Get_readMeText_for_use_of_MBCO_PathNet()
   writeLines(text_lines, complete_readMe_for_MBCO_PathNet_fileName)
   parameterSetting_lines = Get_MBCOAppParameterSettings_for_use_of_MBCO_PathNet()
   writeLines(parameterSetting_lines, complete_parameterSettings_for_MBCO_PathNet_fileName)
}#End - Write MBCO PathNet ReadMe file
