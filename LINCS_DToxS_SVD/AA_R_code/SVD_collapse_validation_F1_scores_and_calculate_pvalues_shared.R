...
clusterEvalQ(parallel_clusters,
             {#Begin - Parallel clusters
               ...
               {#Begin - indexCore
                 ...
                 if (length(unique(real_validations$Correlation_method))!=1) { stop("length(unique(real_validations$Correlation_method))!=1") }
                 if (length(unique(real_validations$Preprocess_data))!=1) { stop("length(unique(real_validations$Preprocess_data))!=1") }
                 if (length(unique(real_validations$Decomposition_method))!=1) { stop("length(unique(real_validations$Decomposition_method))!=1") }
                 if (length(unique(real_validations$CumEigenexpression_cutoff))!=1) { stop("length(unique(real_validations$CumEigenexpression_cutoff))!=1") }
                 if (length(unique(real_validations$SigDEGs_correlation_method))!=1) { stop("length(unique(real_validations$SigDEGs_correlation_method))!=1") }
                   ...
                   {#Begin - indexEA 
                     ...
                     {#Begin - indexEntity
                       ...
                       {#Begin - Select highestF1_highestMedianCorrelationReducedFullData for each subject
                         ...
                         entity_real = entity_real[order(entity_real$Eigenassays_count,decreasing=TRUE),]
                         entity_real = entity_real[order(entity_real$Mean_correlation_reduded_full_data,decreasing=TRUE),]
                         ...
                         {#Begin - Get highest F1 score for each sample in subspace that gives final selection
                           ...
                           {#Begin
                             ...
                             indexCurrentSample = grep(sample,entity_real$Samples_in_cluster) #ok
