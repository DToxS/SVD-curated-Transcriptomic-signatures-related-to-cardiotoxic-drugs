...
{#Begin
  ...
  tasks$Core_no[startIndex:endIndex] = indexCore
...
if (length(fullData_eigenassay_count)!=1) { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": ",entityClass," ",addOrAnalyze_coCulture_data,"(length(fullData_eigenassay_count)!=1)",sep='')) }
...
if ( (length(unique(fullData_collapsed_real_validations$Entity))!=length(unique(collapsed_real_validations$Entity)))
   ...
   { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": ",entityClass," ",addOrAnalyze_coCulture_data,"(length(unique(fullData_collapsed_real_validations$Entity))!=length(unique(collapsed_real_validations$Entity)))",sep='')) }
...
{#Begin - for (indexAllEntities in 1:length(all_entities))
   ...
   {#Begin - Generate reduced data
      ...
      if (length(unique(fS_entity_collapsed_validations$Eigenassays_count))!=1) { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": length(unique(fS_entity_collapsed_validations$Eigenassays_count))!=1",sep='')) }
      if (length(fs_full_entity_collapsed_validations[,1])!=1) { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": (length(fs_full_entity_collapsed_validations[,1])!=1)",sep='')) }
      
      eigenassays_count = unique(fS_entity_collapsed_validations$Eigenassays_count)
      ...
      if (length(current_eigenassays)!=length(unique(current_eigenassays))) { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": Duplicated eigenassays in current_eigenassays",sep='')) }
   ...
   {#Begin - (indexVisualize in 1:length(names(data_for_visualization_list)))
      ...
      default_current_entity_color = "black"
      ...
      current_drug_cellline_labels = replicate(length(current_colnames),"error")
      ...
      {#Begin - Generate colors for single entity dendrograms
         ...
         drug = splitStrings[3]
         {if (drug %in% names(structuralGroups))
         { drugStructure = structuralGroups[[drug]] }
         else { drugStructure = "No structure" }}
         ...
         else {current_drug_labels[indexColNames] = drug } }
         ...
         {#Begin-Generate cell line labels and drug cell line labels for DToxS and all other datasets
             if (  (dataset=="SVD_DEGenes_iPSCdCMs_P0_Signed_minus_log10pvalue")
                  ||(dataset=="SVD_DEGenes_iPSCdCMs_P0_Signed_minus_log10pvalue_no1stSVD")
                  ||(dataset=="SVD_DEGenes_iPSCdCMs_ECCoCulture_Signed_minus_log10pvalue")
                  ||(dataset=="SVD_DEGenes_iPSCdCMs_ECCoCulture_Signed_minus_log10pvalue_no1stSVD"))
             {#Begin - Remove clone from LINCs DToxS cell lines for generation of current_cellline_labels and current_drug_cellline_labels
               splitStrings = strsplit(cellline,"_")[[1]]
               if (length(splitStrings)<=2)
               {#Begin
                  current_cellline_labels[indexColNames] = splitStrings[1]
                  current_drug_cellline_labels[indexColNames] = paste(current_drug_labels[indexColNames],": ",current_cellline_labels[indexColNames],sep='')
               }#End
      
               if (length(splitStrings)==3)
               {#Begin
                  current_cellline_labels[indexColNames] = paste(splitStrings[3]," ",splitStrings[1],sep='')
                  current_drug_cellline_labels[indexColNames] = paste(splitStrings[3]," - ",current_drug_labels[indexColNames],": ",splitStrings[1],sep='')
               }#End
             }#End - Remove clone from LINCs DToxS cell lines for generation of current_cellline_labels and current_drug_cellline_labels
             else
             {#Begin - Generate current_cellline_labels and current_drug_cellline_labels for all other datasets
                current_cellline_labels[indexColNames] = cellline
                current_drug_cellline_labels[indexColNames] = paste(current_drug_labels[indexColNames],": ",cellline,sep='')
             }#End - Generate current_cellline_labels and current_drug_cellline_labels for all other datasets
         }#End - Generate cell line labels and drug cell line labels for DToxS and all other datasets
          
         current_drugClass_labels[indexColNames] = drugClass
         if ((current_entity=="All")||(drugStructure==currentEntity_structure))
         {#Begin
            if (drug %in% names(structuralGroup_colors))
            { current_drugStructure_colors[indexColNames] = structuralGroup_colors[[drug]] }
            else { current_drugStructure_colors[indexColNames] = drug }
         }#End
         ...
         {#Begin
            ...
            {#Begin
              current_drug_cellline_colors[indexColNames] = default_current_entity_color
            }#End
         }#End
      ...
      {#Begin - Set colors and labels for single entity dendrogram
         ...
         v_labels = list(current_cellline_labels,current_drug_cellline_labels,current_drug_labels,current_drug_labels)
         ...
         v_labels = list(current_drug_cellline_labels)
         current_complete_visualization_of_clusterDendrograms_directory = visualization_of_entitySpecific_clusterDendrograms_base_directory
      ...
      {#Begin - Add single entity dendrogram to PDF
          ...
          if (length(unique(current_label_colors))<=1) { stop(paste("Core ",indexCore,", task ",current_task_line$Task_no,": (length(unique(current_label_colors))<=1)",sep='')) }
    ...
    }#End - (indexVisualize in 1:length(names(data_for_visualization_list)))
}#End - for (indexAllEntities in 1:length(all_entities))
