...
if (length(unique(cluster_validations$Dataset))!=1) { stop("(length(unique(cluster_validations$Dataset))!=1)") }
if (length(unique(cluster_validations$Correlation_method))!=1) { stop("(length(unique(cluster_validations$Correlation_method))!=1)") }
if (length(unique(cluster_validations$Preprocess_data))!=1) { stop("(length(unique(cluster_validations$Preprocess_data))!=1)") }
if (length(unique(cluster_validations$Decomposition_method))!=1) { stop("(length(unique(cluster_validations$Decomposition_method))!=1)") }
if (length(unique(cluster_validations$CumEigenexpression_cutoff))!=1) { stop("(length(unique(cluster_validations$CumEigenexpression_cutoff))!=1)") }
...
{#Begin - Generate figure set
   ...
   current_validations$Plot_drugName = "error"   
   indexEntity = 1
   ...
   {#Begin
      ...
      {#Begin
         current_validations$Plot_drugName[indexCurrentEntity] = drug_shortenedFullNames[[entity]]
      }#End
      else
      {#Begin
        current_validations$Plot_drugName[indexCurrentEntity] = entity
      }#End
   }#End

   #current_validations$Plot_drugName = paste("(",current_validations$Entity_samples_count,") ",current_validations$Plot_drugName,sep='')
   ...
   current_validations$Plot_drugName_factor = factor(current_validations$Plot_drugName,levels=unique(current_validations$Plot_drugName))
   ...
   entity_colors = replicate(length(all_entities),missing_color)
   for (indexAllEntities in 1:length(all_entities))
   {#Begin
      current_entity = all_entities[indexAllEntities]
      drugClass = current_entity
      if (current_entity %in% names(drug_drugClass))
      { drugClass = drug_drugClass[[current_entity]] }
      if (drugClass %in% names(drugClass_colors))
      { entity_colors[indexAllEntities] = drugClass_colors[[drugClass]] }
   }#End
   entity_colors = Fill_missing_colors(entity_colors,missing_color)
   ...
   annotate_label_size = 3
   annotate_y = 0.7 * min_y
   bar_width = 0.85
   singleDataPoints_size = 0.25
   singleDataPoints_color = "limegreen"
   singleDataPoints_shape = 19

   indexUnique = which(!duplicated(current_validations_fullData$Entity))
   unique_current_validations_fullData = current_validations_fullData[indexUnique,]
   unique_current_validations_fullData = unique_current_validations_fullData[order(unique_current_validations_fullData$Plot_drugName_factor),]
   fullData_entities_count = length(unique_current_validations_fullData$Entity)
   Plot = ggplot(current_validations_fullData,aes(x=Plot_drugName_factor,y=F1_score_full_data,group=EntityClass,fill=Is_outlier))
   Plot = Plot + geom_bar(stat="identity",width=bar_width)
   if (!TRUE %in% current_validations$Subtract)
   { 
     Plot = Plot + annotate("text",label=unique_current_validations_fullData$Entity_samples_count,x=1:fullData_entities_count,y=annotate_y,size=annotate_label_size)
   ...
   Plot = ggplot(current_validations_fullData,aes(x=Plot_drugName_factor,y=F1_score,group=EntityClass,fill=Is_outlier))
   Plot = Plot + geom_bar(stat="identity",width=bar_width)
   if (!TRUE %in% current_validations$Subtract)
   { 
     Plot = Plot + annotate("text",label=unique_current_validations_fullData$Entity_samples_count,x=1:fullData_entities_count,y=annotate_y,size=annotate_label_size)
   ...
   {#Begin
      ...
      {#Begin
        ...
        {#Begin
           #if (length(indexOutlier)!=1) { stop("length(indexOutlier)!=1") }
        ...
        if (length(plot_validations)>0)
        { plot_validations = rbind(plot_validations,drug_validations[indexNoOutlier[1],]) }
        else
        {#Begin
           plot_validations = drug_validations[indexNoOutlier[1],]
        }#End
     }#End
   }#End
   ...
   indexUnique = which(!duplicated(plot_validations$Entity))
   unique_plot_validations = plot_validations[indexUnique,]
   unique_plot_validations = unique_plot_validations[order(unique_plot_validations$Plot_drugName_factor),]
   plot_entities_count = length(unique_plot_validations$Entity)

   Plot = ggplot(plot_validations,aes(x=Plot_drugName_factor,y=Mean_f1_score_plot,group=EntityClass,fill=Is_outlier))
   Plot = Plot + geom_bar(stat="identity",width=bar_width)
   Plot = Plot + annotate("text",label=unique_plot_validations$Entity_samples_count,x=1:plot_entities_count,y=annotate_y,size=annotate_label_size)
   if (!TRUE %in% current_validations$Subtract)
   { 
     ...
     Plot = Plot + theme(plot.title = element_text(hjust=0.5,face=2,size=title_size))
     Plot = Plot + geom_beeswarm(data=s_sample_validations,aes(y=F1_score,x=Plot_drugName_factor),size=singleDataPoints_size, shape=singleDataPoints_shape, corral="wrap",corral.width=bar_width,color=singleDataPoints_color)
   }
   if (TRUE %in% current_validations$Subtract)
   {
   ...
   {#Begin - Extract cosine similarities
     Col_names = c("Cosine_similarity","Plot_drugName_factor","EntityClass","Is_outlier")
     Col_length = length(Col_names)
     Row_names = ""
     Row_length = length(Row_names)
     cosineSimilarity_dataframe = c()
     
     indexCurrent=1
     for (indexCurrent in 1:dim(current_validations_fullData)[1])
     {#Begin
        current_line = current_validations_fullData[indexCurrent,]
        current_cosineSimilarities = strsplit(current_line$Correlations_reduced_full_data,';')[[1]]
        Row_names = current_cosineSimilarities
        Row_length = length(Row_names)
        add_dF = as.data.frame(array(NA,c(Row_length,Col_length),dimnames = list(Row_names,Col_names)))
        add_dF$Plot_drugName_factor = current_line$Plot_drugName_factor
        add_dF$EntityClass = current_line$EntityClass
        add_dF$Is_outlier = current_line$Is_outlier
        add_dF$Cosine_similarity = as.numeric(current_cosineSimilarities)
        if (length(cosineSimilarity_dataframe)>0) { cosineSimilarity_dataframe = rbind(cosineSimilarity_dataframe,add_dF) }
        else { cosineSimilarity_dataframe = add_dF }
     }#End
   }#End - Extract cosine similarities
   
   Plot = ggplot(current_validations_fullData,aes(x=Plot_drugName_factor,y=Median_correlation_reduded_full_data,group=EntityClass,fill=Is_outlier))
   Plot = Plot + geom_bar(stat="identity",width=bar_width)
   if (!TRUE %in% current_validations$Subtract)
   { 
     Plot = Plot + annotate("text",label=unique_current_validations_fullData$Entity_samples_count,x=1:fullData_entities_count,y=annotate_y,size=annotate_label_size)
     ...
     Plot = Plot + theme(plot.title = element_text(hjust=0.5,face=2,size=title_size))
     Plot = Plot + geom_beeswarm(data=cosineSimilarity_dataframe,aes(y=Cosine_similarity,x=Plot_drugName_factor),size=singleDataPoints_size, shape=singleDataPoints_shape, corral="wrap",corral.width=bar_width,color=singleDataPoints_color)
   }
   if (TRUE %in% current_validations$Subtract)
   { 
   ...
   Plot = ggplot(current_validations_fullData,aes(x=Plot_drugName_factor,y=Entity_samples_count,group=EntityClass,fill=Is_outlier))
   Plot = Plot + geom_bar(stat="identity",width=bar_width)
   if (!TRUE %in% current_validations$Subtract)
   { 
     Plot = Plot + annotate("text",label=unique_current_validations_fullData$Entity_samples_count,x=1:fullData_entities_count,y=-1,size=annotate_label_size)
...
{#Begin
   ...
   {#Begin - Plot F1 scores group by same f1_score_weight
      ...
      Plots = Generate_figure_set(current_validations=f1_validations,dataset_description=dataset_description,drugs_in_drugClass_specific_order=drugs_in_drugClass_specific_order)
   ...
   Plots = Generate_figure_set(current_validations = outlier_validations,
                               dataset_description = dataset_description,
                               drugs_in_drugClass_specific_order = drugs_in_drugClass_specific_order)
