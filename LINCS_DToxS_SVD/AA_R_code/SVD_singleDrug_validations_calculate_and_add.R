  ...
  {#Begin
    ...
    {#Begin
      ...
      {#Begin
        ...
        if (table_current_entities[indexCurrentTable]>=2)
        {#Begin
          ...
          cluster_validation_line$CumEigenexpression_cutoff = cum_eigenexpression_fraction
          ...
          {#Begin 
            if (length(unique(unique(current_samples[indexCurrentEntity_in_cluster]*1E5)))>0) { stop("current_samples[indexCurrentEntity_in_cluster] are not unique")}
          ...
          {#Begin - Calculate cosine similarities
            ...
            if (!grepl(currentCoreTask_Entity,current_fullData_columnName)) { stop("(!grepl(currentCoreTask_Entity,current_fullData_columnName))") }
            ...
            if (!grepl(currentCoreTask_Entity,current_reducedData_columnName)) { stop("(!grepl(currentCoreTask_Entity,current_reducedData_columnName))") }
