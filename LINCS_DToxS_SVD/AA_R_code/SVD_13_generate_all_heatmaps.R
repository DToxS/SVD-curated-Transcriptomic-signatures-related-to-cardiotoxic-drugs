...
{#Begin - Generate drug-specific subspace data and combine into one matrix: Combined_subspace_data
   ...
   {#Begin
      ...
      outlierCellLines = current_collapsed_real_validation$Final_selection_for_sample[indexOutlier]
      ...
      {#Begin - Add to all reduced dataset
        ...
        add_data_current = Data_current[,index_current_entity_cols,drop=FALSE]
        current_add_colnames = colnames(add_data_current)
        if (length(outlierCellLines)>0)
        {#Begin
           indicesOutlier = replicate(length(outlierCellLines),0)
           for (indexOutlierCellLine in 1:length(outlierCellLines))
           {#Begin
              outlierCellLine = outlierCellLines[indexOutlierCellLine]
              indicesOutlier[indexOutlierCellLine] = grep(paste("[.]",outlierCellLine,"[.]",sep=''),current_add_colnames)
           }#End
           indicesNoOutlier = 1:length(current_add_colnames)
           indicesNoOutlier = indicesNoOutlier[!indicesNoOutlier %in% indicesOutlier]
           current_add_colnames[indicesOutlier] = paste(current_add_colnames[indicesOutlier],".Outlier",sep='')
           current_add_colnames[indicesNoOutlier] = paste(current_add_colnames[indicesNoOutlier],".Regular",sep='')
        }#End
        if (length(outlierCellLines)==0)
        {#Begin
...
{#Begin - Function: Get_all_color_and_label_vectors
  ...
  missing_color_color = "gray60"
  
  ## Generate colors - END
  current_drug_colors = replicate(length(current_colnames),missing_color_color)
  current_drug_labels = replicate(length(current_colnames),"error")

  current_drugClass_colors = replicate(length(current_colnames),missing_color_color)
  current_drugClass_labels = replicate(length(current_colnames),"error")

  current_cellline_colors = replicate(length(current_colnames),missing_color_color)
  current_cellline_labels = replicate(length(current_colnames),"error")

  current_sigDEGs_colors = replicate(length(current_colnames),missing_color_color)
  ...
  {#Begin
    ...
    if (length(indexCurrentDrugCellline_in_summary)!=1) { stop("Drug is missing in summary data frame") }
    ...
    { full_drugName = full_drugNames[[drug]] }
    else { full_drugName = drug } }
    
    if (outlier=="Regular"){ current_drug_labels[indexColNames] = full_drugName }
    if (outlier=="Outlier"){ current_drug_labels[indexColNames] = paste("Outlier - ",full_drugName,sep='') }
    ...
    if ( (is.nan(current_colorIndex))
        |(is.na(current_colorIndex))
        |(current_colorIndex==0)) { current_colorIndex = 1}
  ...
  current_drug_colors = Fill_missing_colors(current_drug_colors,missing_color_color = missing_color_color)
  current_cellline_colors = Fill_missing_colors(current_cellline_colors,missing_color_color = missing_color_color)
  current_drugClass_colors = Fill_missing_colors(current_drugClass_colors,missing_color_color = missing_color_color)
  current_sigDEGs_colors = Fill_missing_colors(current_sigDEGs_colors,missing_color_color = missing_color_color)
...
if (dim(Data)[1]!=dim(Combined_subspace_data)[1]) { stop("dim(Data) does not match dim(Combined_subspace_data)") }
if (dim(Data)[2]!=dim(Combined_subspace_data)[2]) { stop("dim(Data) does not match dim(Combined_subspace_data)") }
...
{#Begin
   ...
   labels_and_colors_list = Get_all_color_and_label_vectors(current_colnames=col_labels_in_dendrogram,current_task_line=current_task_line,summary=summary)
