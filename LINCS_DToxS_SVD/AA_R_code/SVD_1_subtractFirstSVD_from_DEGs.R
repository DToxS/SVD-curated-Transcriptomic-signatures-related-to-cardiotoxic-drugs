...
input_datasets = names(inputDataset_inputSVDDataset_list)
...
{#Begin
   ...
   if (!input_dataset %in% names(substract_svds))
   {#Begin
      print(paste(input_dataset,": No eigenarrays are defined for removal",sep=''))
      next;
   }#End
   ...
   {#Begin
      ...
      if (!dir.exists(ouput_r_gene_expression_directory)) { dir.create(ouput_r_gene_expression_directory) }
