...
{#Begin
  directory = paste(lincs_results_directory,"/",dataset,"/1_DEGs/",sep='')
  ...
  {#Begin - Adjust drugClasses
    ...
    {#Begin
      ...
      drugClass = "Unknown"
      if (current_drug %in% names(drug_drugClass))
      { drugClass = drug_drugClass[[current_drug]] }
      Summary$Drug_type[indexS] = drugClass
    }#End
  }#end
  
  indexMissing = which(!Summary$Drug_type %in% drugClassOrder)
  Summary$Drug_type_factor = factor(Summary$Drug_type,levels = c(drugClassOrder,unique(Summary$Drug_type[indexMissing])))
  ...
  {#begin
    ...
    fullDrugName = current_drug
    if (current_drug %in% names(full_drugNames))
    { fullDrugName = full_drugNames[[current_drug]] }
    Summary$Treatment_full_name[indexRow] = paste(fullDrugName," (",drug_table[[current_drug]],") ",sep='')
  }#End
