...
{#Begin - Calculate eigenassay_correlation_sigDEGs_correlation
  ...
  {#Begin
    missing_colNames = paste(unique(plate_summary$Colname[indexRemove]),collapse="; ",sep='')
    stop(paste("Missing data column names in summary (1_DEG directory):\n",missing_colNames),sep='')
  }#End
  ...
  if ((length(equal_columnNames)!=1)&(equal_columnNames[1]==FALSE))
  {#Begin
    indexMissing = which(!currentEigenassay_correlations$Columnname %in% plate_summary$Colname)
    if (length(indexMissing)>0)
    {#Begin
      missingColnames = paste(unique(currentEigenassay_correlations$Columname[indexMissing]),collapse="; ",sep='')
      stop(paste("Missing data column names in summary (1_DEG directory): ",missingColnames,sep=''))
    }#End
    stop("Column names are not equal in data and summary (1_DEGs directory)")
  }#End
