...
namespace Enrichment
{
    ...
    class Enrichment2018_results_line_class
    {
        ...
        private void Set_sampleName_from_splitStrings(string[] splitStrings)
        {
        ...
        public string Lincs_plate
        {
            ...
            set
            {
                ...
                Set_sampleName_from_splitStrings(splitStrings);
            }
        }
        ...
        public Drug_type_enum Lincs_drug_type
        {
            get
            {
                string[] splitStrings = this.Sample_name.Split('-');
                return (Drug_type_enum)Enum.Parse(typeof(Drug_type_enum),splitStrings[splitStrings.Length - 2]);
            }
            set
            {
                string[] splitStrings = this.Sample_name.Split('-');
                splitStrings[splitStrings.Length - 2] = value.ToString();
                Set_sampleName_from_splitStrings(splitStrings);
            }
        }
    ...
    class Enrichment2018_results_class
    {
        ...
        public void Keep_only_lincs_drugTypes(params Drug_type_enum[] keep_lincsDrugTypes)
        {
            Dictionary<Drug_type_enum, bool> keep_lincsDrugTypes_dict = new Dictionary<Drug_type_enum, bool>();
            foreach (Drug_type_enum keep_lincsDrugType in keep_lincsDrugTypes)
            {
                keep_lincsDrugTypes_dict.Add(keep_lincsDrugType, true);
            }
            int enrichment_length = Enrichment_results.Length;
            Enrichment2018_results_line_class enrichment_results_line;
            List<Enrichment2018_results_line_class> keep = new List<Enrichment2018_results_line_class>();
            ...
            {
                enrichment_results_line = this.Enrichment_results[indexE];
                if (keep_lincsDrugTypes_dict.ContainsKey(enrichment_results_line.Lincs_drug_type))
                {
                    keep.Add(enrichment_results_line);
