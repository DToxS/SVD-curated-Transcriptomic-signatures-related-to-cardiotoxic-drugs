...
- The results folders starting with 'MBCO_desktop_application' contain files that can be read using our MBCO application (mbc-ontology.org) to investigate which expressed genes map to identified pathways. Use the 'Read data menu', select 'Read all files in directory', copy-paste the whole directory into the 'Read all files in directory' text box, select 'Example 1'- (or 'Custom 1'-) column names, press 'Read' and then 'Analyze'. In the results folder, open those networks with yED that contain the same name twice (e.g., 'Mbco_human_Contraction_Contraction.graphml').
