...
clusterEvalQ(parallel_clusters,
             {#Begin - Parallel clusters
               ...
               {#Begin - indexCoreTask
                 ...
                 permutation_no_string = current_task_line$Permutation_no
                 ...
                 {#Begin
                       ...
                       {#Begin - Identify outlier for each entity
                         ...
                         {#Begin
                           ...
                           {#Begin
                             ...
                             outlier_line$Eigenassays_count = unique(entity_validations$Eigenassays_count)
                             outlier_line$Median_correlation_reduced_full_data = unique(entity_validations$Median_correlation_reduded_full_data)
                             outlier_line$Mean_correlation_reduced_full_data = unique(entity_validations$Mean_correlation_reduded_full_data)
                   ...
                   {#Begin - if outliers were found for at least one drug
                     ...
                     {#Begin - Define significance for outlier
                       ...
                       outliers$UniqueIdentifier = paste(outliers$Entity,"-",outliers$Outlier,outliers$Reference_valueType,sep='')
                       indexUnique = which(!duplicated(outliers$UniqueIdentifier)) #up to one outlier each F1 score weight for each drug and reference value type
                       indexSignificant = which(outliers$Significant==outlier_significant_label)
                       indexSigUnique = indexUnique[indexUnique %in% indexSignificant]
                       outliers$Final_selection[indexSigUnique] = outlier_finalSelection_yes_label
                       outliers$UniqueIdentifier = NULL
                   ...
                   if (length(finalSelectionOutliers))
                   {#Begin - Add final selection for outlier to collapsed_cluster_validation
                     ...
                     {#Begin
                       ...
                       if (length(outlier_celllines)!=1) { rm(outlier_celllines) }
                       ...
                       if (length(indexCollapsed_outlier)!=1) { rm(collapsed_cluster_validations) }
                   ...
                   {#Begin - Generate outlier figures for each reference_valueType, if outliers were found
                     ...
                     {#Begin - Generate outlier figures for each reference_valueType
                       ...
                       }#End - Dcoument how many drug of each drug target induce outlier response
